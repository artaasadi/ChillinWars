...
class AI(RealtimeAI):
    ...
    def initialize(self):
        ...
        self.orders.add_order(base, [AmmoType.GoldenTankShell, AmmoType.TankShell, AmmoType.TankShell])
            ...
            # age nis faqat bargard
                ...
                if sum(self.orders.required_materials_list.values()) == 0:
                    ...
                    if base.units[UnitType.GoldenTank].health == 0 or self.current_cycle + 20 >= self.world.max_cycles:
                        # TODO ehtemalan shart docomie avaz beshe
                    ...
                    '''if base.units[UnitType.GoldenTank].health > 600:
                        amir_ammo.append(AmmoType.GoldenTankShell)
                    elif base.units[UnitType.Tank].health > 1500:
                        amir_ammo.append(AmmoType.TankShell)
                    elif other_base.units[UnitType.GoldenTank].health == 0 and base.units[UnitType.Soldier].health != 0:
                        amir_ammo.append(AmmoType.RifleBullet)
                    elif other_base.units[UnitType.GoldenTank].health != 0 and base.units[UnitType.Mortar].health != 0:
                        amir_ammo.append(AmmoType.MortarShell)

                    if base.units[UnitType.Tank].health > 2000:
                        amir_ammo.append(AmmoType.TankShell)
                    else:
                        amir_ammo.append(AmmoType.RifleBullet)
                    if base.units[UnitType.HeavyMachineGunner].health > 1200:
                        amir_ammo.append(AmmoType.HMGBullet)
                    else:
                        amir_ammo.append(AmmoType.RifleBullet)'''
