...
class AI(RealtimeAI):
    ...
    def decide(self):
        ...
        # ------ Material
            ...
            if (self.wagent_ammo_loaded == True) and ((sum(wagent.ammos_bag.values()) == 0 and wagent.position.index != 1) or (sum(wagent.ammos_bag.values()) != 0)):
                self.warehouse_agent_move(forward=False)
                
            elif (self.wagent_ammo_loaded == True) and sum(wagent.ammos_bag.values()) == 0 and wagent.position.index == 1:
                self.wagent_ammo_loaded = False
                material = base.warehouse.materials[wagent.position]
                if material.type in self.orders.required_materials_list and self.orders.required_materials_list[material.type] != 0:
                    if material.count != 0:
            ...
            if sum(self.orders.required_materials_list.values()) == 0:
                    if (other_base.units[UnitType.Soldier].health + other_base.units[UnitType.HeavyMachineGunner].health + other_base.units[UnitType.Mortar].health) > 1800:
                        if base.units[UnitType.Soldier].health > 1200:
                            self.orders.add_order(base, [AmmoType.GoldenTankShell, AmmoType.RifleBullet, AmmoType.MortarShell])
                        else:
                            self.orders.add_order(base, [AmmoType.GoldenTankShell, AmmoType.HMGBullet, AmmoType.MortarShell])
                    else:
        ...
        if current_fagent_position == ECell.BacklineDelivery:
            ...
            # factory agent vasayele vas sakhte order ro bardare ta jaii k mitune
                ...
                while self.orders.orders_list != [] and i < len(self.orders.orders_list):
                    ...
                    if sum(base.factory.c_mixture_formulas[self.orders.orders_list[i]].values()) + bag_capacity <= 15 and list(temp_dict.values()) <= list(base.backline_delivery.materials.values()):
                ...
                elif (MachineStatus.AmmoReady in list(machine.status for machine in base.factory.machines.values()) or (self.orders.fagent_orders != [] and MachineStatus.Idle in list(machine.status for machine in base.factory.machines.values()))):
            ...
            elif (sum(base.backline_delivery.materials.values()) == 0 or self.fagent_bag_is_full == True) and ((self.orders.fagent_orders != [] and MachineStatus.Idle in list(machine.status for machine in base.factory.machines.values())) or MachineStatus.AmmoReady in list(machine.status for machine in base.factory.machines.values())):
                self.factory_agent_move(forward= True) 
        ...
        # ------ Factory
            ...
            # Idle
                ...
                # age order nadare ...
                    ...
                    elif (MachineStatus.AmmoReady in list(machine.status for machine in base.factory.machines.values())) or base.factory.machines[Position(7)].construction_rem_time == 1 or base.factory.machines[Position(8)].construction_rem_time == 1 or base.factory.machines[Position(9)].construction_rem_time == 1:
            ...
            # Working
                ...
                elif self.orders.fagent_orders != [] and MachineStatus.Idle in list(machine.status for machine in base.factory.machines.values()):
